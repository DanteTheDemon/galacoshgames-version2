

const devlogPosts = [
  {
    id: 1,
    title: "Alpha Release: No Safe Tile is finally playable!",
    excerpt: "After months of hard work and countless coffees, we are thrilled to announce that the first playable Alpha of 'No Safe Tile' is available to our playtesters. Here's a breakdown of the new mechanics.",
    content: "We added new enemy AI, revamped the movement system, and completely overhauled the lighting. Stay tuned for more updates as we approach the beta milestone.",
    date: "May 02, 2026",
    author: "Emir, Founder",
    image: "images/content.png"
  },
  {
    id: 2,
    title: "Redesigning the Main Menu",
    excerpt: "We realized our main menu wasn't standing out against the background. Here is a deep dive into our iterative design process for creating a more readable main menu.",
    content: "We focused on silhouette readability and high contrast color palettes to ensure the player never loses track of the character in chaotic levels.",
    date: "April 15, 2026",
    author: "Emir, Founder",
    image: "images/Main Menu.png"
  },
  {
    id: 3,
    title: "The Sound of Galacosh: Powering Our Games with Suno AI",
    excerpt: "A behind-the-scenes look at how every track in the Galacosh Games universe is composed using Suno AI..",
    content: "As a small indie team, resource management is just as important as creativity. By integrating Suno into our audio pipeline, we are able to produce vibrant, studio-quality soundtracks that perfectly match the fast-paced energy of our games. This post explains our creative prompt engineering process, how we match AI-generated beats to the gameplay of No Safe Tile, and why embracing AI tools allows us to focus entirely on building better gaming experiences.",
    date: "March 28, 2026",
    author: "Emir, Founder",
    image: "images/suno.png"
  },
  {
    id: 4,
    title: "Our Debut is Here: No Safe Tile is Officially Launching!",
    excerpt: "The wait is over! Galacosh Games is incredibly excited to announce our official debut with our very first title: No Safe Tile.",
    content: "It has been an amazing journey bringing this mind-bending ecosystem puzzle to life. We poured all our energy into creating a game where deep strategy meets the asymmetric rules of nature. The grid is set, the animals are ready, and we can't wait for you to jump in, test your skills, and experience the challenge we've built for you!",
    date: "February 10, 2026",
    author: "Emir, Founder",
    image: "images/galacoshdark.png"
  }
];

